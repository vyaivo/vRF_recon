function vRFRecon_IEM_trnAllJitter_acrossSess(subList,voi_names,varargin)
...
for v = 1:length(voi_names)
    ...
    for s = 1:length(subList)  % loop over unique subjects
        ...
        for c = 1:nconds
            ...
            for l = 1:size(locs,1)
                lrows = find(ismember(origpos,locs(l,:),'rows'));
                posbins(c,lrows) = l;
            end
        ...
        fname = sprintf('%srecons/%s_TrnAllJitter_acrossSess_%s.mat',...
            root,subList{s},voi_names{v});
        fprintf('Saving %s\n',fname);
        save(fname,'chanResp','reconw','chanX','chanY','FWHM','stim_pos',...
