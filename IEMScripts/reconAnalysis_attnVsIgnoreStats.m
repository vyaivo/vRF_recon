%%% reconAnalysis_attnVsIgnoreStats.m
...
statfn = fullfile(root,fitdir,'reconAnalysis_statswANOVA.mat');

%%

datfn = sprintf('%s%s/AllSubs_CompiledRecons_%s.mat',root,fitdir,trnfix);
load(datfn);
alls = size(reconFits);
...
mean_overallerr = mean(reshape(reserr(:,1:6,:,:),1,[]));
ci_overallerr = prctile(reshape(reserr(:,1:6,:,:),1,[]),[2.5 97.5]);
