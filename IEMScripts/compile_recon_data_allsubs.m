function compile_recon_data_allsubs(trnfix)
...
rdir = 'recons';
fitdir = 'reconfits';
fitfix = 'acrossSess'; %trnfix = 'AllJitter';
sublist = {'AA','AI','AL','AP','AR','AT','AU'};
voilist = {'V1','V2','V3','V3AB','V4','IPS0'};
ns = length(sublist);
nv = length(voilist);

savefn = sprintf('%s%s/AllSubs_CompiledRecons_%s.mat',root,fitdir,trnfix);
...
reconSubAvg = squeeze(mean(allRecons));
...
save(savefn,'allRecons','reconSubAvg','reconFits','rdat','binlabels','eccbini',...
    ...
    'nbins','nc','npar','fov','res','fitErr','-v7.3');
fprintf('Saved %s!\n',savefn);