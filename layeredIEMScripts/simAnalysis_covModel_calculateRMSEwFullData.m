% simAnalysis_covModel_calculateRMSEwFullData.m
...
niters = 500;   % resample the data & fit for this many iterations
...
fn = fullfile(root,'reconfits','AllSubs_CompiledRecons_TrnAllJitter.mat');
load(fn,'locs','attendlocs','fov','res','voilist','nv','nbins','nc','npar');
atlocs = attendlocs;
np = npar - 1;
npos = size(locs,1);
...
if doFit
    simAnalysis_fitSimRecons(niters,voilist,fov,'_covNoise_');
else    
    if ~exist('bigfitfn','file')
        for v = 1:nv
            fitfn = sprintf('%s%s/sim_covNoise_RMSE_fit_%s.mat',root,...
                simdir,voilist{v});
            load(fitfn,'allfits','niters');

            if ~exist('parcombs','var')
                thisfn = sprintf(...
                    '%s%s/sim_covNoise_vRFRecon_allSubsNoOutliers_%s.mat',...
            ...
            % now tmp is 500 x 11 x 2 x 51 x 5
        ...
        save(bigfitfn,'modeldat','parcombs','mlabels','niters','nm','-v7.3');
    else
        load(bigfitfn);
    end
end
...
fn2 = fullfile(root,'reconfits','AllSubs_CompiledRecons_goodvRFs_TrnAllJitter.mat');
load(fn2, 'reconFits');
reconFits(:,:,:,:,3) = rad2fwhm(reconFits(:,:,:,:,3));
reconFits = squeeze(nanmean(reconFits,2)); % avg across subs
...
for v = 1:nv
    ...
    mi2 = sqrt( nanmean(nanmean(nanmean(iterrmse.^2,2),3),4) ); clear iterrmse;
    for m = 1:nm-1
        iterrmse = repmat(fullrec(v,:,:,:),niters,1,1,1) - ...
            squeeze(modeldat(m,v,:,:,:,:));
        mi1 = sqrt( nanmean(nanmean(nanmean(iterrmse.^2,2),3),4) );        
       
        pval_eachm(v,m) = 2*min([nanmean(mi2-mi1 < 0),...
                nanmean(mi2-mi1 > 0)]);
    end
end
...
%%
savefn = sprintf('%s%s/layeredSimModel_covNoise_fitRMSE.mat',root,simdir);
save(savefn,'modelRMSE','ciRMSE','rs','reducedRMSE',...
