% simAnalysis_calculateRMSEwFullData.m
...
%%
% fit options
doFit = 1;      % fit the layered IEM recons
...
bigfitfn = sprintf('%s%s/all_sim_fitsWModel.mat',root,simdir);
% labels of each recon parameter (position,size,amplitude,baseline)
parlab = {'p','s','a','b'};

%% find out which subjects we had to exclude
fn = dir(sprintf('%s%s/sim_vRFRecon_allSubsNoOutliers_*.mat',root,simdir));
...
save(bigfitfn,'modeldat','fullrec','reducedrec','parcombs','mlabels',...
    'niters','nm','condlocs','nbins','locs','condlocs','atlocs','-v7.3');
...
%%
savefn = sprintf('%s%s/layeredSimModel_fitRMSE.mat',root,simdir);
